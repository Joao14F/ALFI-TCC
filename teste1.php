...
$sql = "SELECT `Capa`, `Id modelo` FROM `modelo` LIMIT $offset, $resultados_por_pagina";
...
if ($res && mysqli_num_rows($res) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        if (isset($row['Capa'])) { // Verifica se a chave 'Capa' está definida
            $id = $row['Id modelo']; // Atribui o valor do ID à variável $id
            $caminho_imagem = $row['Capa'];
            echo '<a href="Acesso.php?id=' . $id . '">';
